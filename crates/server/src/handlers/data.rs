...
pub async fn nav(
    ...
    // elsewhere; this is a read, so `mutating: false`. Task 9 migrates this
    // check onto `Scoped` — the `st.pool` shim is sanctioned here in the
    // meantime.
...
pub async fn positions(
    ...
    // needs this explicit guard. Task 9 migrates it onto `Scoped`.
    super::portfolios::ensure(&st.pool, pid, false).await?;
