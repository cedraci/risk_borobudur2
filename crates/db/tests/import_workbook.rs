...
async fn import_upsert_and_duplicate_semantics() {
    ...
    let o1 = db::repo::import_workbook(&pool, 1, "sample.xlsx", "sha-1", &wb).await.unwrap();
    assert!(!o1.duplicate);
    ...
    let nav = db::repo::nav_rows(&pool, 1).await.unwrap();
    ...
    let o2 = db::repo::import_workbook(&pool, 1, "sample.xlsx", "sha-1", &wb).await.unwrap();
    assert!(o2.duplicate);
    assert_eq!(o2.import_id, o1.import_id);
    assert_eq!(db::repo::nav_rows(&pool, 1).await.unwrap().len(), 344);

    // same file, new sha -> re-import replaces the snapshot (still 111 rows, one date)
    let o3 = db::repo::import_workbook(&pool, 1, "sample2.xlsx", "sha-2", &wb).await.unwrap();
    assert!(!o3.duplicate);
    assert!(o3.div_ops_replaced); // equal nav_date counts as >=
    let dates = db::repo::position_dates(&pool, 1).await.unwrap();
    assert_eq!(dates, vec![d(2026, 7, 24)]);
    let pos = db::repo::positions_for(&pool, 1, d(2026, 7, 24)).await.unwrap();
    assert_eq!(pos.len(), 111);
    assert!(pos.iter().any(|p| p.isin == "GRS145003000"));

    let imports = db::repo::imports_list(&pool, 1).await.unwrap();
