...
fn evidence_round_trips_through_calamine() {
    let e = EmirEvidence {
        ...
        summary: vec![SummaryRow {
            ...
            threshold_eur: 3e9,
            avg_otc_eur: 1.5e9,
            pct_of_threshold: 0.5,
            verdict: "watch".into(),
            avg_total_eur: 2.5e9,
        }],
        months: vec![
            MonthRow { label: "Interest-rate derivatives".into(), month: "2026-06".into(), snapshot_date: Some("2026-06-26".into()), total_eur: Some(1.0e7), otc_eur: Some(4.0e6) },
        ...
        kpis: vec![KpiRow { month: "2026-07".into(), unconfirmed_over_5d: 3, reconciliation: "not_applicable".into(), disputes: 2, note: "".into() }],
    ...
    names.sort();
    assert_eq!(names, ["Contrats", "KPI", "Seuils"], "unexpected sheet set: {names:?}");
    ...
    // Summary table: header row then the one class row. Sentinel values are
    // distinct and non-zero so a column swap (e.g. avg OTC <-> avg total, or
    // pct <-> verdict) would be caught here.
    assert_eq!(cell(&s, 5, 0), "Class");
    assert_eq!(cell(&s, 6, 0), "Interest-rate derivatives");
    assert_eq!(cell(&s, 6, 1), "3000000000"); // threshold
    assert_eq!(cell(&s, 6, 2), "1500000000"); // avg OTC
    assert_eq!(cell(&s, 6, 3), "0.5"); // pct of threshold (raw fraction; see fold-in 3)
    assert_eq!(cell(&s, 6, 4), "watch"); // verdict
    assert_eq!(cell(&s, 6, 5), "2500000000"); // avg total
    ...
    assert_eq!(cell(&s, 9, 3), "10000000"); // total EUR
    assert_eq!(cell(&s, 9, 4), "4000000"); // OTC EUR (distinct from total)
    ...
    assert_eq!(cell(&k, 1, 1), "3"); // unconfirmed_over_5d
    assert_eq!(cell(&k, 1, 2), "not_applicable");
    assert_eq!(cell(&k, 1, 3), "2"); // disputes
}
